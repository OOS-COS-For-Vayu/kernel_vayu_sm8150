 ...
 * Copyright (C) 2025, Anymore Project <anymore.dev13@gmail.com>.
 */
...
static void waltgov_set_iowait_boost(struct waltgov_cpu *wg_cpu, u64 time, unsigned int flags)
{
	if (flags & SCHED_CPUFREQ_IOWAIT) {
		if (wg_cpu->iowait_boost_pending)
			return;

		wg_cpu->iowait_boost_pending = true;

		if (wg_cpu->iowait_boost) {
			wg_cpu->iowait_boost <<= 1;
			if (wg_cpu->iowait_boost > wg_cpu->iowait_boost_max)
				wg_cpu->iowait_boost = wg_cpu->iowait_boost_max;
		} else {
			wg_cpu->iowait_boost = wg_cpu->wg_policy->policy->min;
			wg_cpu->iowait_boost_max = wg_cpu->wg_policy->policy->max;
		}
		...
		if (delta_ns > TICK_NSEC) {
			...
			wg_cpu->iowait_boost_pending = false;
		}
	}
}

static void waltgov_iowait_boost(struct waltgov_cpu *wg_cpu, unsigned long *util, unsigned long *max)
{
	unsigned int boost_util = wg_cpu->iowait_boost;
	unsigned int boost_max = wg_cpu->iowait_boost_max;
	...
	if (wg_cpu->iowait_boost_pending) {
		...
		wg_cpu->iowait_boost >>= 1;
		if (wg_cpu->iowait_boost < wg_cpu->wg_policy->policy->min) {
			wg_cpu->iowait_boost = 0;
			return;
		}
	}
...
#define DEFAULT_CPU0_RTG_BOOST_FREQ 1785600
#define DEFAULT_CPU4_RTG_BOOST_FREQ 2419200
#define DEFAULT_CPU7_RTG_BOOST_FREQ 2956800
